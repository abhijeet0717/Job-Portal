...
const JobDescription = () => {
  const { singleJobById } = useSelector(store => store.job);//store the job applied in redux
    ...
    const fetchSingleJob = async () => {
      try {
        ...
        if (res.data.success) {
          ...
          //so we populted the application field of job(coitailn application id normally) , so we can get the applicant id 
          setIsApplied(res.data.job.applications.some(application => application.applicant === authUser?._id)); // Ensure the state(redux state) is in sync with fetched data
        }
      } catch (error) {
        console.log(error);
      }
